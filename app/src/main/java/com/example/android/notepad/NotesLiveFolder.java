...
public class NotesLiveFolder extends Activity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        if (LiveFolders.ACTION_CREATE_LIVE_FOLDER.equals(action)) {
            ...
            // 修复：兼容所有API版本（保留ShortcutIconResource，添加过时注解）
            // 说明：LiveFolders仅支持通过EXTRA_LIVE_FOLDER_ICON传递ShortcutIconResource，无其他替代方案
            @SuppressWarnings("deprecation")
            Intent.ShortcutIconResource foldericon = Intent.ShortcutIconResource.fromContext(
                    this,
                    // 若没有live_folder_notes图标，替换为系统内置图标（推荐）
                    android.R.drawable.ic_menu_edit
                    // 若有自定义图标，使用：R.drawable.live_folder_notes
            );
